...
void merge_sort(int q[], int l, int r)
{
    ...
    int k = 0, i = l, j = mid + 1;
    while (i <= mid && j <= r)
        //temp在外面定义个全局变量，或使用动态分配内存，只是使用完成后需要手动释放内存
...
```C++
// C = A + B, A >= 0, B >= 0
vector<int> add(vector<int> &A, vector<int> &B)// 使用引用传递参数，以避免在调用过程中发生不必要的拷贝，提高效率
{
...
int main()
{
	...
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++)
			s[i][j] = s[i - 1][j] + s[i][j - 1] - s[i - 1][j - 1] + a[i][j];//前缀和的计算
	while (q--)
	{
		int x1, x2, y1, y2;
		scanf("%d%d%d%d", &x1, &x2, &y1, &y2);
		printf("%d\n", s[x2][y2] - s[x1 - 1][x2] - s[x1][x2 - 1] + s[x1 - 1][x2 - 1]);//计算部分和
	}
	return 0;
}
```
