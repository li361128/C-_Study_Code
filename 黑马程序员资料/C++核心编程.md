...
//int func(int a, int b = 10, int c )是错误的，c在有默认值b后须有默认值


//2. 如果函数声明有默认值，函数实现的时候就不能有默认参数  即main函数调用里及函数定义里不能重新设置其他常数
...
//函数返回值不可以作为函数重载条件
//int func(double a, int b)  //与上面的void不同
//{
//	cout << "func (double a ,int b)的调用！" << endl;
//}
...
void test02() {
	...
	//Person p5(p4);//编译器会认为重复声明Person p5了
}
...
public:
	...
	Person operator+(const Person& p) {
		Person temp;/*前面设置了有参构造导致编译器不产生默认构造，
		但这里需要默认构造，故要在前面添加默认构造的函数Person() {};*/
...
public:
	int getResult(string oper)
	{
		...
		//如果在普通实现中  要提供新的运算，需要修改源码
        //但在真实开发中 提倡 开闭原则
        //开闭原则： 对拓展进行开放，对修改进行关闭->用多态
	}
public:
	int m_Num1;
	int m_Num2;
