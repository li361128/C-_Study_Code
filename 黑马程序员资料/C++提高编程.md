...
class Person
{
	...
	//全局函数配合友元  类外实现
    //加空模板参数列表
    //如果是全局函数 是类外实现， 需要让编译器提前知道这个函数的存在，即把函数实现
...
- 前面使用`const` 表示返回值为`const`
  ...
  //getptr() 函数返回的指针是一个指向常量整数的指针，而不是一个普通的整数指针
  int main()
  {
...
总结：map中所有元素都是成对出现，插入数据时候要使用**对组**
...
void test01()
{
	...
	//第四种插入方式
	m[4] = 40; //不建议用它（[]）来进行插入，输入错误会创建新的数，不太好。 其用途： 可以利用key访问到value
	printMap(m);
...
* 利用仿函数可以指定map容器的排序规则
* 对于**自定义**数据类型，map必须要指定排序规则,同set容器
...
* 当数据以**键值对**形式存在，可以考虑用`map` 或 `multimap`
...
void test01()
{
	...
	vTarget.resize(v.size()); // 目标容器需要提前开辟空间,不然程序会崩
...
总结：面试题中如果出现查找相邻重复元素，记得用`STL`中的adjacent_find算法
...
**transform()和copy()区别：**
	transform()可以将源容器的元素进行类似multiplies()操作之后再传入目的容器，而copy()只是复制过去不能进行修改操作（他只有三个参数）。
